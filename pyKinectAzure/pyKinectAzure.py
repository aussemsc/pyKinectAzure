...
class pyKinectAzure:
    ...
    def transform_depth_to_color(self,input_depth_image_handle, color_image_handle):
        ...
        return transformed_depth_image_handle, transformed_image
    ...
    def transformation_depth_image_to_point_cloud(self, input_depth_image_handle, calibration_type):
        device_calibration = _k4a.k4a_calibration_t()
        
        # Get desired image format
        image_format = _k4a.K4A_IMAGE_FORMAT_CUSTOM
        width = self.image_get_width_pixels(input_depth_image_handle)
        height = self.image_get_height_pixels(input_depth_image_handle)
        stride = 6*width
        
        # Get the camera calibration
        self.device_get_calibration(self.config.depth_mode,self.config.color_resolution,device_calibration)
        
        # Set the transformation handle
        transformation_handle = self.transformation_create(device_calibration)
        
        # Create the image handle        
        transformed_depth_image_handle = _k4a.k4a_image_t()
        self.image_create(image_format,width,height,stride,transformed_depth_image_handle)
        
        # Define from which camera perspective we do the calibration, 
        #calibration_type = 1 # 0= depth, 1 = color
        self.k4a.k4a_transformation_depth_image_to_point_cloud(transformation_handle,input_depth_image_handle,calibration_type,transformed_depth_image_handle)
        
        #Get image size
        image_size = self.image_get_size(transformed_depth_image_handle)
        #Get pointer to start of image data
        buffer_pointer = self.image_get_buffer(transformed_depth_image_handle)
    ...
    def transform_color_to_depth(self, input_color_image_handle, input_depth_image_handle):
        ...
        self.k4a.k4a_transformation_color_image_to_depth_camera(transformation_handle,input_depth_image_handle,input_color_image_handle, transformed_color_image_handle)
